...
- **Titkárnő mode (Posta)**: the former Döntés-sor. Per-item flow: 🔊 Felolvasás (free browser `speechSynthesis`, hu-HU) reads sender+subject+gist+thread tail; a plain textarea takes the user's RAW dictated decision (they use system-wide Wispr Flow - the in-app 🎤 SpeechRecognition button is optional sugar); "✍ Fogalmazd meg" POSTs to `/api/rephrase`, which spawns the local claude CLI (`CLAUDE_BIN`, stdin prompt, cwd=tmpdir, strict JSON out) to write the FINAL letter in Áron's register (style file + the bot's 3 drafts as register reference; em-dash stripped server-side). The result shows as a highlighted `.po-draft--gen` box with the same Másolás/Levélíróba actions. The dictation block is available on every opened row, not just in Titkárnő mode. Draft boxes are indented with intent-colored left borders (`.po-draft--0/1/2`).
